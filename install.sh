...
# KlowCode installer
# Usage: curl -fsSL https://raw.githubusercontent.com/paoloanzn/klowcode/main/install.sh | bash
...
REPO="https://github.com/paoloanzn/klowcode.git"
INSTALL_DIR="$HOME/klowcode"
...
ART
  printf "${RESET}"
  printf "${DIM}  KlowCode — AI coding agent, fully unlocked${RESET}\n"
  echo ""
}
...
build_binary() {
  info "Building KlowCode (all experimental features enabled)..."
  cd "$INSTALL_DIR"
...
link_binary() {
  ...
  ln -sf "$INSTALL_DIR/cli-dev" "$link_dir/klowcode"
  ok "Symlinked: $link_dir/klowcode"
...
printf "    ${CYAN}klowcode${RESET}                           # interactive REPL\n"
printf "    ${CYAN}klowcode -p \"your prompt\"${RESET}           # one-shot mode\n"
echo ""
printf "  ${BOLD}Set your API key:${RESET}\n"
printf "    ${CYAN}export ANTHROPIC_API_KEY=\"sk-ant-...\"${RESET}\n"
echo ""
printf "  ${BOLD}Or log in with Claude.ai:${RESET}\n"
printf "    ${CYAN}klowcode /login${RESET}\n"
echo ""
printf "  ${DIM}Source: $INSTALL_DIR${RESET}\n"
printf "  ${DIM}Binary: $INSTALL_DIR/cli-dev${RESET}\n"
printf "  ${DIM}Link:   ~/.local/bin/klowcode${RESET}\n"
echo ""
